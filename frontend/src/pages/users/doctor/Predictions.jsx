...
export default function Predictions() {
  ...
  const runPrediction = async () => {
    ...
    try {
      ...
      const res = await authenticatedFetch(
        `${VITE_API_URL}/ml/${selectedModel}/db`,
        {
          method: "POST",
  ...
  const runSensitivity = async () => {
    ...
    try {
      ...
      const res = await authenticatedFetch(
        `${VITE_API_URL}/ml/${selectedModel}/sensitivity/db`,
        {
          method: "POST",
          body: JSON.stringify({ patientId: patient.id }),
        }
      ...
      if (!res.ok)
        throw new Error(
          `Failed to get sensitivity analysis for ${selectedModel}`
        );
      const data = await res.json();
  ...
  const runShap = async () => {
    ...
    try {
      ...
      const res = await authenticatedFetch(
        `${VITE_API_URL}/ml/${selectedModel}/shap/db`,
        {
          method: "POST",
          body: JSON.stringify({ patientId: patient.id }),
        }
  ...
  const downloadReport = async () => {
    ...
    try {
      const res = await authenticatedFetch(
        `${VITE_API_URL}/reports/${patient.id}`
      );
  ...
  const computeYDomain = (data, seriesKeys) => {
    if (!data || !seriesKeys || seriesKeys.length === 0)
      return ["auto", "auto"];
    let min = Infinity,
      max = -Infinity;
    data.forEach((row) =>
        ...
        <div style={{ width: "100%", height: 300 }}>
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data}>
              ...
              <Tooltip
                formatter={(v) =>
                  typeof v === "number" ? [v.toFixed(4), "Value"] : [v, "Value"]
                }
              />
              <Legend />
              {original_x != null && (
  ...
  if (!patient && id)
    return (
      <div className="p-6 text-center text-red-600">Patient not found</div>
    );
  if (user?.role !== "doctor")
    return (
      <div className="p-6 text-center text-red-600">
        Access denied. Only doctors can view predictions.
      </div>
    );
          ...
          {!id && (
            <div className="space-y-4">
              ...
              <select
                value={patient?.id || ""}
                onChange={(e) =>
                  setPatient(patients.find((p) => p.id === e.target.value))
                }
        ...
        <div className="flex gap-4 mt-6 flex-wrap">
          ...
          {patient && (
            <button
              ...
              className="flex items-center gap-2 py-3 px-6 rounded-md font-medium text-white bg-blue-600 hover:bg-blue-700"
            >
          ...
          {!sensitivityResult.sensitivity && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-md p-4 mb-4">
              <p className="font-medium text-yellow-800">
                Sensitivity Analysis Data Format Issue
              </p>
              <p className="text-sm mt-1">
                Expected `sensitivity` object but didn't find one.
              </p>
            </div>
          )}
          {charts.length > 0 ? (
      ...
      {shapResult && (
        <div className="bg-white rounded-lg shadow-lg border border-blue-200 p-8">
          ...
          {!shapResult.shap || Object.keys(shapResult.shap).length === 0 ? (
            ...
            Object.entries(shapResult.shap).map(
              ([subModelKey, subModelData]) => {
                ...
                const maxVal = Math.max(
                  ...shapChartData.map((d) => Math.abs(d.value))
                );
