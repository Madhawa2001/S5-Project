...
export default function Requests() {
  ...
  const handleRequest = async (id, action, role) => {
    try {
      setProcessingId(id)
      if (action === "approve") {
        const approveResp = await authenticatedFetch(`http://localhost:5000/admin/approve/${id}`, { method: "POST" })
        if (!approveResp.ok) throw new Error("Failed to approve request")

        const assignRoleResp = await authenticatedFetch("http://localhost:5000/admin/assign-role", {
          method: "POST",
        ...
        <div className="bg-white rounded-lg shadow-md border border-green-200 p-6">
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              {requests.map((req) => (
                <div key={req.id} className="border border-green-200 rounded-lg p-4 hover:shadow-md transition-shadow flex justify-between items-center">
                  <div>
                    <div className="flex items-center gap-3 mb-1">
                      <h3 className="text-lg font-semibold text-green-900">{req.name}</h3>
